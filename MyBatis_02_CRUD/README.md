...
#### 一、使用SqlSession的inster，selectList，update，delete等对表执行CRUD操作——基于XML的实现
1、定义sql映射xml文件
...
例如namespace="com.shi.mapping.userMapper"就是com.shi.mapping(包名)+userMapper(userMapper.xml文件去除后缀)
 -->
<mapper namespace="com.shi.mapping.demo01.userMapper">
...
<mappers>
   <!-- 注册UserMapper映射接口-->
   <mapper resource="com/shi/mapping/demo02/userMapper.xml"/>
</mappers>
...
public class TestCRUD {
    ...
    public void testAdd(){
        ...
        /**
         * 映射sql的标识字符串，
         * com.shi.mapping.demo01.userMapper是userMapper.xml文件中mapper标签的namespace属性的值，
         * addUser是insert标签的id属性值，通过insert标签的id属性值就可以找到要执行的SQL
         */
        String statement = "com.shi.mapping.demo01.userMapper.addUser";//映射sql的标识字符串
        User user = new User();
        user.setName("用户孤傲苍狼");
        user.setAge(20);
        //执行插入操作
    ...
    public void testUpdate(){
         ...
         */
        String statement = "com.shi.mapping.demo01.userMapper.updateUser";//映射sql的标识字符串
        User user = new User();
        user.setId(3);
        user.setName("孤傲苍狼");
        user.setAge(25);
    ...
    public void testDelete(){
         ...
         */
        String statement = "com.shi.mapping.demo01.userMapper.deleteUser";//映射sql的标识字符串
        //执行删除操作
    ...
    public void testGetAll(){
         ...
         */
        String statement = "com.shi.mapping.demo01.userMapper.getAllUsers";//映射sql的标识字符串
...
```
#### 二、使用UserMapperIByXml对表执行CRUD操作——基于XML的实现
1、定义sql映射xml文件
userMapper.xml文件的内容如下：
```xml
<?xml version="1.0" encoding="UTF-8" ?>
...
例如namespace="com.shi.mapping.userMapper"就是com.shi.mapping(包名)+userMapper(userMapper.xml文件去除后缀)
 -->
<mapper namespace="com.shi.mapping.demo02.UserMapperIByXml">
    <!-- 在select标签中编写查询的SQL语句， 设置select标签的id属性为getUser，id属性值必须是唯一的，不能够重复
    使用parameterType属性指明查询时使用的参数类型，resultType属性指明查询返回的结果集类型
    resultType="com.shi.mapping.User"就表示将查询结果封装成一个User类的对象返回
    User类就是users表所对应的实体类
...
```
需要说明的是，我们不需要针对UserMapperIByXml接口去编写具体的实现类代码，这个具体的实现类由MyBatis帮我们动态构建出来，我们只需要直接拿来使用即可。
3、单元测试类代码如下：
```java
public class TestCRUD {
...
```
#### 三、使用UserMapperIByNote对表执行CRUD操作——基于注解的实现
...
<configuration>
    ...
    <mappers>
        <!-- 注册UserMapper映射接口-->
        <mapper class="com.shi.mapping.demo03.UserMapperIByNote"/>
    </mappers>
